...
class Resources(dj.Lookup):
    ...
    """
    contents = [['r6g.4xlarge',80,100,7,None,None,None],
                ['r6g.3xlarge',50,75,7,None,None,None],
                ['r6g.xxlarge',35,50,7,None,None,None],
...
class ResourceModel(dj.Lookup):
    ...
    def model(self, model, key_hash, table):
       ...
       if model == 'neurd':
            ...
            if (len(diamonddownload.DecimatedMesh & f'segment_id={key_segment}')>0) and (len(diamondprocess.SomaExtraction & f'segment_id={key_segment}')>0): 
